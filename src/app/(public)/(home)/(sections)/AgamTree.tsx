...
const TreeMarkerLeft = ({ className, title, subtitle, number }: { className: string, title: string, subtitle: string, number: number }) => {
    const ref = useRef(null);
    const [isVisible, setIsVisible] = React.useState(false);
            ...
            <AnimatePresence>
                {isVisible && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        transition={{ duration: 1 }}
                        className="absolute -top-2 border-r-8 border-primary-ui bg-white/50 right-16 md:right-44 sm:right-36 sm:pl-3 p-1 md:min-w-52 w-28 sm:min-w-40"
                    >
                        <Typography variant='h4' className='w-full italic text-typography max-sm:text-sm'>{title}</Typography>
                        <Typography variant='p' className='!sm:text-base w-full font-mono italic text-typography/70 !text-xs leading-none'>{subtitle}</Typography>
                    </motion.div>
