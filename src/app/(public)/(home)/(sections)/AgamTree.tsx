...
const TreeMarker = ({ className, title, subtitle, active, setActive, number }: { className: string, title: string, subtitle: string, active: number, setActive: Dispatch<SetStateAction<number>>, number: number }) => {

    return (
        <div className={`absolute  ${className}`} >
            <div onClick={() => setActive((prev: number) => prev === number ? 0 : number)} className="bg-[#E3DFD5] cursor-pointer sm:w-16 sm:h-16 w-12 h-12 p-2 text-center flex justify-center items-center rounded-full">
                <Typography variant='h4' className='!sm:text-sm !text-xs leading-tight w-full italic text-typography'>{title}</Typography>
            </div>
            <div className={`absolute -top-5 border-l-8 border-primary-ui bg-white/50 left-16 md:left-44 sm:left-36 sm:pl-3 p-1 md:min-w-52 w-28 sm:min-w-40 ${active === number ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300 `}>
