...
const TopSection = ({ coverImage }: { coverImage: CoverImageDTO | null }) => {
    return (
        <section className="w-screen h-full relative flex bg-fixed flex-col md:flex-row items-center justify-center md:justify-around mx-auto"
        // style={{
        //     backgroundImage: `url(${coverImage?.media.url ?? "/static/placeholder.png"})`,
        ...
        >
            <Image
                src={"/static/bhagwan/banner.png"}
                alt={coverImage?.media.alt ?? "Image for presentation"}
                width={2000}
                height={2000}
                className="w-screen h-full max-h-screen object-cover"
            />
